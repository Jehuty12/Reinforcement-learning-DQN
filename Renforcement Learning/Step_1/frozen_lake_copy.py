...
# Paramètres pour la décroissance logistique d'epsilon
epsilon_max = 1.0
E0 = 30000     # Point d'inflexion : épisode où la décroissance est la plus rapide
tau = 3000     # Contrôle de la pente : plus petit -> transition plus rapide

def compute_epsilon(episode):
    """
    Fonction logistique pour calculer epsilon en fonction de l'épisode.
    Supprime `epsilon_min` pour permettre une descente complète vers 0.
    """
    return epsilon_max / (1 + np.exp((episode - E0) / tau))

def plot_q_table(ax, Q, grid_size=8, trap_states=None):
    """
    Affiche la Q-table sous forme de heatmap avec les meilleures actions représentées par des flèches.
    Gestion des valeurs nulles pour éviter l'affichage vide.
    """
    ax.clear()
    ...
    max_q = np.max(Q)
    if max_q > 0:
        normalized_Q = Q / max_q
    else:
        normalized_Q = np.zeros_like(Q)  # Si Q est vide, on garde des 0
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            if trap_states is not None and state in trap_states:
                continue  #Cache les flèches pour les cases pièges
...
def run():
    ...
    ax_frame = axes[0, 0]   # Dernière frame
    ax_qtable = axes[0, 1]  # Q-table
    ax_rewards = axes[1, 0] # Rolling rewards
    ax_steps = axes[1, 1]   # Moyenne des steps
    ax_epsilon = axes[1, 2] # Évolution d'epsilon
    ...
    env = gym.make('FrozenLake-v1', map_name='8x8', is_slippery=False, render_mode="rgb_array", desc=generate_random_map(size=8))
    ...
    for episode in range(num_episodes):
        ...
        while not (terminated or truncated) and episode_steps < max_steps:
            if np.random.rand() < compute_epsilon(episode):
        ...
        if len(rewards_per_episode) < window_size:
            rolling_sum = np.sum(rewards_per_episode)
        else:
            rolling_sum = np.sum(rewards_per_episode[-window_size:])
        ...
        eps_values.append(compute_epsilon(episode))
        ...
        if episode % update_interval == 0 and episode > 0:
            ...
            # Mise à jour de la Q-table et de la colorbar
